...
**데이터베이스 시작하기1**  
- `CREATE DATABASE` 데이터베이스 생성 -> `USE` 작업할 데이터베이스 선택
- `CREATE TABLE` 테이블 설계 (컬럼별 데이터 타입, 기본 키 지정)
- `DESC` 테이블 구조 확인, `SHOW` 데이터베이스나 테이블 목록  
- `DROP` 데이터베이스나 테이블 삭제
...
**데이터 타입**
- 숫자 타입에는 정수를 저장하는 `INT` 와 정확한 소수를 저장하는 `DECIMAL` 등이 있다.
- 문자열 타입에는 가변 길이 `VARCHAR` 와 고정 길이 `CHAR` 가 있으며 대부분 `VARCHAR` 를 사용한다.
- 날짜와 시간 타입에는 `DATE` `DATETIME` `TIMESTAMP` 가 있고 `DATETIME` 을 권장한다.

**제약 조건**
- 제약 조건은 데이터의 무결성을 보장하기 위해 테이블에 특정 규칙을 강제하는 장치다.
- `NOT NULL` 은 값이 비어있는 것을 허용하지 않는다.  
- `UNIQUE` 는 중복된 값을 허용하지 않는다.  
- `PRIMARY KEY` 는 테이블의 각 행을 유일하게 식별하며 `NOT NULL` 과 `UNIQUE` 특징을 모두 갖는다. 
