...
쇼핑몰을 운영한다고 가정했을 때, 쇼핑몰이 성장함에 따라 이런 문제들은 점점 더 심각해지고, 결국에는 쇼핑몰 운영 자체가 불가능해질 수도 있다.
...
즉, 데이터베이스 관리 시스템은 서비스의 소중한 데이터를 안전하고 효율적으로 관리하여, 필요로 하는 정보를 언제든지 정확하게 얻을 수 있도록 도와주는 핵심 기반 시설이라고 할 수 있다.
...
지난 시간에는 파일 시스템으로 데이터를 관리할 때 발생하는 다양한 문제점들에 대해 알아보았다. 데이터가 중복되고, 일관성이 깨지고, 찾기도 어렵고, 보안도 허술하고, 여러 사람이 동시에 쓰기도 힘들고, 고장 나면 복구도 어렵고... 쇼핑몰이 이런 식으로 운영된다면 정말 큰일일 것이다.
...
6강. 데이터와 정보
- DBMS를 통해 데이터를 저장, 관리하고, 원하는 정보를 추출할 수 있다

7강. 데이터베이스 관리 시스템이 필요한 이유
- 텍스트, 엑셀로 데이터를 저장하면 문제가 생긴다. 
- 이전의 문제 - 데이터 중복과 불일치, 흩어진 데이터의 구조화, 무결성 제약조건 적용이 번거로움, 동시성 제어의 난관, 보안 유출, 회복과 백업의 어려움 등
- 이런 문제들을 해결하기 위해 DBMS가 등장했다

8강. 데이터베이스 관리 시스템
- DBMS는 DB를 다루는 프로그램이고, SQL을 표준적인 질의 언어로 사용한다
- 테이블 자체를 설정하거나 관계를 설정할 때는 DDL
- 테이블의 데이터를 조작할 때는 DML

9강. 관계형 데이터베이스 vs NoSQL
- RDB는 테이블과 테이블의 관계를 통해 데이터를 저장하고 관리한다.
- NoSQL은 비정형 데이터 등을 다룬다. (RDB 이외의 나머지를 통칭)
- 실무에서는 RDB를 기본으로, NoSQL은 옵션으로 사용한다.

10강. 관계형 데이터베이스 종류
- 대부분의 RDB는 SQL 표준을 준수하므로, 하나를 제대로 배우면 다른 것도 사용 가능하다.

11강. 관계형 데이터베이스 핵심 개념
- 기본 키 (PK) - 모든 테이블에 설정하는 게 원칙, 고유성과 NOT NULL을 지켜야 한다.
- 외래 키 (FK) - 참조 무결성을 보장해야 한다.
